...
void GUI_ch_ShowFont16(u8 x,u8 y,u8 *s,u8 mode)
{
	u8 i,j,k,tmp;
	u16 num;
	num = sizeof(a_cfont16)/sizeof(typFNT_GB16);
  for(i=0;i<num;i++)
	{
		if((a_cfont16[i].Index[0]==*s)&&(a_cfont16[i].Index[1]==*(s+1)))
		{
			for(j=0;j<32;j++)
			{
				if(mode)
				{
					tmp = a_cfont16[i].Msk[j];
				}
				else
				{
					tmp = ~(a_cfont16[i].Msk[j]);
...
void GUI_ShowCHinese1(u8 x,u8 y,u8 hsize,u8 *str,u8 mode)
{ 
	while(*str!='\0')
	{
		if(hsize == 16)
		{
			GUI_ch_ShowFont16(x,y,str,mode);
		}
		else if(hsize == 24)
		{
			GUI_ShowFont24(x,y,str,mode);
